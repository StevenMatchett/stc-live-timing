...
export const LiveTiming = (props) =>{
    ...
    return (
        <React.Fragment>
            {time && <DriverModal open={true} driver={time} close={()=>{setTime(null)}}/>}
            <div class="flex-grid">
                {data && <AutoXTable class="col" data={data["PAX"]} name={"PAX"} showModal={(time)=>{setTime(time)}}  />}
                {data && <AutoXTable class="col" data={data["RAW"]} name={"RAW"} showModal={(time)=>{setTime(time)}} />}
                {data && Object.keys(data).map(className => {
                    if (className === "RAW" || className === "PAX") {
                        return null
                    };
                    return (
                        <AutoXTable class="col" data={data[className]} name={className} showModal={(time)=>{setTime(time)}}  />
                    )
                })}
            </div>
        </React.Fragment>
